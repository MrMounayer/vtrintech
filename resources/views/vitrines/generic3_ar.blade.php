...
<head>
    ...
    <title>{{ $vitrine->name ?? 'صفحة شخصية' }}</title>
    <style>
...
<body class="{{ $style ?? 'theme-default' }}">
    ...
    <section class="hero">
        <div class="hero-content">
            <p class="greeting">مرحبًا، أنا</p>
            <h1 class="hero-name">{{ $vitrine->name ?? 'اسمك' }}<span>.</span></h1>
    ...
    <section class="services-section" id="services">
        <div class="section-header">
            ...
            <h2 class="section-title">ما أفعله</h2>
        </div>
        ...
        @else
            <p style="text-align:center;color:var(--text-muted);padding:60px;">الخدمات قادمة قريبًا...</p>
        @endif
    </section>
    ...
    <section class="carousel-section">
        <div class="section-header">
            <p class="section-label">أعمال</p>
            <h2 class="section-title">أعمال حديثة</h2>
        </div>
                ...
                <div id="image-carousel" class="carousel-track">
                    @foreach($carouselImages as $image)
                        <div class="carousel-card">
                            <img
                                src="{{ $image->getUrl() }}"
                                alt="{{ $image->name ?: 'صورة من المعرض' }}"
                                onclick="openFullscreen(this)"
                                data-fullscreen-src="{{ $image->getUrl() }}"
                                {{-- data-title="{{ $image->name ?? 'صورة المشروع' }}" --}}
                                {{-- data-description="{{ $image->custom_properties['description'] ?? 'لمحة عن العمل المنجز.' }}" --}}
                                style="cursor:pointer;"
                            />
                            {{-- <div class="carousel-caption">
                                <strong>{{ $image->name ?? 'صورة المشروع' }}</strong>
                            </div> --}}
                        </div>
                    @endforeach
                </div>
        ...
        @else
            <p style="text-align:center;color:var(--text-muted);padding:60px;">المعرض قادم قريبًا...</p>
        @endif
    ...
    <section class="cta-section">
        <div class="cta-content">
            <h2 class="cta-title">{{ !empty($vitrine->cta) ? $vitrine->cta : "لنبتكر شيئًا مدهشًا" }}</h2>
            <p class="cta-subtitle">{{ $vitrine->cta_description ?? "هل أنت مستعد لبدء مشروعك التالي؟ أنا على بعد رسالة واحدة." }}</p>
